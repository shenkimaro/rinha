...
class Image {
	...
	public static function showImg($filename, $widthMax = 0) {
		...
		if ($widthMax > 0 && ($imgSize[0] > $widthMax)) {
			$proporcao = $widthMax * 100 / $imgSize[0];
			$newheight = ($proporcao * 0.01) * $imgSize[1];
			$dst = imagecreatetruecolor($widthMax, $newheight);
			imagecopyresampled($dst, $img, 0, 0, 0, 0, $widthMax, $newheight, $imgSize[0], $imgSize[1]);
			$img = $dst;
		}
		...
		if ($image_type == IMAGETYPE_PNG) {
			...
			imagecopyresampled($thumb, $source, 0, 0, 0, 0, $widthMax, $newheight, $imgSize[0], $imgSize[1]);
		...
		header('Content-Type: image/' . $arq_tipo);
