...
class TDG {
	...
	public static function getConnectionDbLog($array = array()) {
		...
		if ($log == null) {
			die("Sem Conexao de log com " . $_SERVER['HTTP_HOST'] . ' ou ' . $_SERVER['SERVER_ADDR']);
		}
	...
	public function genericQueryWithCache($sql, $lifeTime = CacheLbr::_TIME_DEFAULT, $folder = '') {
		...
		if ($result == null) {
			...
			if (count($result) > 0) {
				$cache->add($namedKey, serialize($result), $folder);
			}
		}
		return $result;
	}
	 ...
	 * @return array
	 * @throws Exception
	 * @todo fazer cache funcionar ao selecionar e ao alterar dados chamar o clearCache
	 */
	public function genericQuery($sql) {
		try {
			$cache = new CacheLbr();
			if ($cache->isCacheOn()) {
				if ($this->getFromCache($sql) != null) {
					return $this->getFromCache($sql);
				}
				$result = $this->addToCache($sql);
			} else {
	...
	private function getFromCache($sql): ?array {
		$cache = new CacheLbr();
		$namedKey = $sql;
		$result = $cache->get($namedKey, CacheLbr::_TIME_DEFAULT);
		if ($result == null) {
			return $result;
		}
		return null;
	...
	private function addToCache($sql) {
		$cache = new CacheLbr();
		$ex = $this->db->query($sql);
		$result = $this->db->fetch_all($ex);
		$namedKey = $sql;
		if (count($result) > 0) {
			$cache->add($namedKey, serialize($result));
		}
		return $result;
	}
	 ...
	 * @param DTO $dto
	 * @param boolean $insetPK
	 * @param array $log
	 * @return DTO
	 * @throws Exception
	 */
	public function insert(DTO $dto, $insetPK = false, array $log = array()) {
		$isSerial = TRUE;
		try {
			$this->getCurrSeq($dto);
			$this->sql = $this->buildInsert($dto, $insetPK);
		} catch (Exception $e) {
			$this->sql = $this->buildInsert($dto, $insetPK, TRUE);
			$isSerial = FALSE;
		}
		...
		if (in_array($dto->$method(), ['', DTO::NULL])) {
			if (!$isSerial) {
	...
	public function formatSearchText($field, $var) {
		...
		$value = str_replace(" ", "%", $var);
		$where = "(unaccent($field) ilike unaccent('%" . $value . "%') ";
