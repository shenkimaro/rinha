...
class View {
	...
	const ENGINE_TINYBUTSTRONG_3_13_2 = 'TinyButStrong3_13_2';
	...
	public function loadTemplate() {
		...
		if (!$this->templateExistsFullPath()) {
			$rest = new Restful();
			$data['message'] = 'Rota/saida inexistente para essa aplicacao.';
	 ...
	 * @param string $blockName Nome do bloco no html
	 */
	public function setPage($page = 1, $blockName) {
		if (trim($page) == '' || (int) $page <= 0)
			$this->page = 1;
		else
			$this->page = $page;
